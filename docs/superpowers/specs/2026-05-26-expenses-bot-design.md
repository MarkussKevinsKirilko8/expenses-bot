...
The bot mirrors the user's language: replies (confirmations, error messages,
query answers) are produced in whatever language the user wrote or spoke in.
Claude detects the language and responds in kind — static strings like the
confirmation prompt are kept language-neutral (emoji + parsed fields) or
generated by Claude in the matching language.
